    ...
    function (Controller, JSONModel) {
        ...
        return Controller.extend("zpj.pro.sk.sd.supplychain.zprosupchain.zprosupplychain.i18n.i18nView1", {
            ...
            onRenegotiationButtonPress: function () {
                var vValidation = 0;
                var aTablePayload = this.getView().getModel("oRequestModel").getData().NAV_SC_HEADER_REQUEST.results;
                var len = this.getView().getModel("oRequestModel").getData().NAV_SC_HEADER_REQUEST.results.length;
                ...
                if (vValidation === 1) {
                    var payload = {
                        ...
                        "Action": "BPRENG",
                        "NAV_SC_HEADER_REQUEST": []
                    }
                    var nLen = this.getView().getModel("oRequestModel").getData().NAV_SC_HEADER_REQUEST.results.length;
                    for (let index = 0; index < nLen; index++) {
                        delete this.getView().getModel("oRequestModel").getData().NAV_SC_HEADER_REQUEST.results[index].__metadata
                        payload.NAV_PM_REQUEST.push(
                            {
                                "Pafno": this.getView().getModel("oRequestModel").getData().Pafno,
                                "Posnr": (index+1).toString(),
                                "ApprovedBuyingprice": this.getView().getModel("oRequestModel").getData().NAV_SC_HEADER_REQUEST.results[index].ApprovedBuyingprice
                            }
                        )
                    }
            ...
            onNegotiationNotPossibleButtonPress: function () {
                ...
                var payload = {
                    "Pafno": this.getView().getModel("oRequestModel").getData().Pafno,
                    "Action": "NBPRENG",
                    "NAV_SC_HEADER_REQUEST": this.getView().getModel("oRequestModel").getData().NAV_SC_HEADER_REQUEST.results
                }
